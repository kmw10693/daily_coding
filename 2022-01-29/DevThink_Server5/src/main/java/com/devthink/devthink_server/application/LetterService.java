...
public class LetterService {
     ...
     * 사용자로부터 메시지 정보를 받아 메시지 리스트에서 메시지를 보냅니다.
             ...
             */
    public Letter addMessage(User sender, User target, LetterSendData dto) {
        // RoomId가 0인 경우 프로필에서 보낸 것이고, 0 이상이면 메시지 리스트에서 보낸 것으로 구분한다.
        if(dto.getRoomId() == 0) {
            int existChat = letterRepository.existChat(sender.getId(), target.getId());
            // 프로필에서 보낸 메시지중 메시지 내역이 없어서 첫 메시지가 될 경우 구분
            if(existChat == 0) {
                // 메시지 DB에서 roomId의 최댓값을 가져옵니다.
                Long maxRoom = letterRepository.maxRoom();

                if(maxRoom == null) {   // room_id 기본 값은 1로 설정합니다.
                    dto.changeRoomId(1L);
                }
                else {
                    dto.changeRoomId(maxRoom + 1);  // (room_id의 최댓값 + 1)을 유저가 주고받은 방 번호로 설정합니다.
                }
            }
            else {    // 기존에 메시지 내역이 있다면 해당 방 번호를 가져옵니다.
                List<Letter> letters = letterRepository.selectRoom(target.getId(), sender.getId(), PageRequest.of(0, 1));
                dto.changeRoomId(letters.get(0).getRoom().getId());
            }
        }
     ...
     */
    public ArrayList<LetterModificationData> messageList(Long userId) {
        // 메시지 리스트에 나타낼 것들: 가장 최근 메시지, 보낸 사람 id
        List<Letter> list = letterRepository.getMessageList(userId);
        List<LetterModificationData> letterList = new ArrayList<>();
